...
DROP PROCEDURE IF EXISTS deleteFromTicket;
DELIMITER $$
CREATE PROCEDURE deleteFromticket(IN n_ticket_id BIGINT UNSIGNED)
	BEGIN 
		DELETE FROM ticket WHERE ticket_id = n_ticket_id;
	END $$
DELIMITER ;
...
DROP PROCEDURE IF EXISTS getSpecificUser;
DELIMITER $$
CREATE PROCEDURE getSpecificUser(IN n_name VARCHAR(64))
	BEGIN 
		SELECT * FROM users WHERE `name` = n_name;
	END $$
DELIMITER ;
...
CREATE PROCEDURE selectClosedTicketsByID(IN n_user_id BIGINT UNSIGNED)
    BEGIN         
		SELECT * FROM problem NATURAL JOIN (SELECT ticket.ticket_id, priority, date_created, status, user_id, 
        group_concat(name) as technicians FROM ticket LEFT OUTER JOIN (SELECT ticket_id, name, tech_assigned_to 
        FROM users JOIN ticketAssignment ON user_id = tech_assigned_to) AS T ON t.ticket_id = ticket.ticket_id 
        GROUP BY ticket.ticket_id, priority, date_created, status, user_id HAVING user_id = n_user_id) as T WHERE 
        `status`="CLOSED" OR `status`="DENIED";
END $$
DELIMITER ;
